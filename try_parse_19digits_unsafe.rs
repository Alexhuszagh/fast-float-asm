...
pub fn try_parse_19digits(s: &mut AsciiStr<'_>, x: &mut u64) {
    // SAFETY: safe since s cannot be empty when first and step are called.
    unsafe {
        while *x < MIN_19DIGIT_INT && !s.is_empty() && s.first_unchecked().is_ascii_digit() {
            let digit = s.first_unchecked() - b'0';
