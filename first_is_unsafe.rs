...
pub trait ByteSlice: AsRef<[u8]> {
    ...
    unsafe fn first_is_unchecked(&self, c: u8) -> bool {
        // SAFETY: safe as long as self is not empty
        unsafe { self.first_unchecked() == c }
    }

    /// Check if the first character in the slice is equal to c1 or c2.
    unsafe fn first_is2_unchecked(&self, c1: u8, c2: u8) -> bool {
        // SAFETY: safe as long as self is not empty
        let c = unsafe { self.first_unchecked() };
        c == c1 || c == c2
    }

    /// Check if the first character in the slice is an ASCII digit.
    unsafe fn first_isdigit_unchecked(&self) -> bool {
        // SAFETY: safe as long as self is not empty
        unsafe { self.first_unchecked() }.is_ascii_digit()
    }
    ...
    fn first_is(&self, c: u8) -> bool {
        // SAFETY: slice cannot be empty, so index at 0 is safe.
        // TODO(ahuszagh) Should be able to replace with map?
        !self.is_empty() && unsafe { self.first_is_unchecked(c) }
    }

    /// Check if the first character in the slice is equal to c1 or c2.
    fn first_is2(&self, c1: u8, c2: u8) -> bool {
        // SAFETY: slice cannot be empty, so index at 0 is safe.
        !self.is_empty() && unsafe { self.first_is2_unchecked(c1, c2) }
    }

    /// Bounds-checked test if the first character in the slice is a digit.
    fn first_isdigit(&self) -> bool {
        // SAFETY: self cannot be empty, so index at 0 is safe.
        !self.is_empty() && unsafe { self.first_isdigit_unchecked() }
    }
